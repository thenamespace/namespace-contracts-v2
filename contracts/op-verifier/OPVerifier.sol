...
contract OPVerifier is IEVMVerifier {
    error OutputRootMismatch(OPWitnessProofType proofType, uint256 index, bytes32 expected, bytes32 actual);
    ...
    error DeprecatedProof(OPWitnessProofType proofType, uint256 disputeGameIndex);
    ...
    function getDisputeGameOutput(uint256 index) internal view returns (bytes32) {
        IDisputeGameFactory disputeGameFactory = optimismPortal.disputeGameFactory();
        ...
        (GameType gameType, Timestamp gameCreationTimeRaw, IDisputeGame gameProxy) =
        ...
        if (block.timestamp - gameCreationTime < disputeGameMinAge) {
            revert GameTooEarly(index, block.timestamp - gameCreationTime, disputeGameMinAge);
        }
        ...
        returns (bytes[] memory values)
    {
        ...
        if (outputRoot != expectedRoot) {
            revert OutputRootMismatch(opData.proofType, opData.index, expectedRoot, outputRoot);
        }
        return
